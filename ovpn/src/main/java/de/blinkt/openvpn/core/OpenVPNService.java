...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener, IOpenVPNServiceInternal {
    ...
    private static String state = "DISCONNECTED";
    boolean flag = false;
    ...
    private void showNotification(final String msg, String tickerText, @NonNull String channel,
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
            Intent launchIntent = getPackageManager().getLaunchIntentForPackage("secure.cpe.vpn");
...
//                    humanReadableByteCount(out, false,getResources())) + " - " + humanReadableByteCount(diffOut / OpenVPNManagement.mBytecountInterval, false, getResources()) + "/s";
    ...
    private static Consumer<String> stateFun = null;

    public static void addOnStateChangeListener(Consumer<String> handler) {
        stateFun = handler;
    }
    private void sendMessage(String state) {
        ...
        if (stateFun != null) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                stateFun.accept(state);
            }
        }
    }
    //sending message to main activity
    private static Consumer<Triple<String, String, String>> detailFun = null;

    public static void addOnDetailChangeListener(Consumer<Triple<String, String, String>> handler) {
        detailFun = handler;
    }
    ...
    private void sendMessage(String duration, String lastPacketReceive, String byteIn, String byteOut) {
        ...
        if (detailFun != null) {
            Triple<String, String, String> stateData = new Triple<>(duration, byteIn, byteOut);
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                detailFun.accept(stateData);
            }
        }
    }
